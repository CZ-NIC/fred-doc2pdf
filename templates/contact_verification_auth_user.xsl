...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="contact_auth">
    <document>
      <template pageSize="(21cm, 29.7cm)" leftMargin="2.0cm" rightMargin="2.0cm" topMargin="2.0cm" bottomMargin="2.0cm" title="contact new user authentication" showBoundary="0" author="CZ.NIC">
  ...
  <xsl:template name="onePage">
    ...
    <para style="title"><xsl:value-of select="$contact_loc/str[@name='Authentication new user contact']"/></para>
    ...
    <para style="main"><xsl:value-of select="$contact_loc/str[@name='this letter approves the succefful creation of contact account with these dates:']"/></para>
    <spacer length="0.5cm"/>
  ...
  <para style="main"><xsl:value-of select="$contact_loc/str[@name='To access into your contact account is necessary to verify your identity with a pair of pins PIN1 and PIN2:']"/>
  </para>
  <spacer length="0.2cm"/>

  <para style="main"><xsl:value-of select="$contact_loc/str[@name='PIN1 was sent to your email address']"/></para>
  <para style="main"><xsl:value-of select="$contact_loc/str[@name='PIN2 is:']"/>&SPACE;<b><xsl:value-of select="auth/codes/pin2"/></b></para>
  <spacer length="0.2cm"/>

    <para style="main"><xsl:value-of select="$contact_loc/str[@name='For activate your account click on a link you received by email.']"/></para>
  <spacer length="0.8cm"/>
    
    <para style="main"><xsl:value-of select="$contact_loc/str[@name='Your team CZ.NIC.']"/></para>
  <spacer length="0.6cm"/>
